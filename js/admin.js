/* ============================================================
   JCA Treinamentos — Admin (Firebase com autenticação de admin)
   ============================================================ */
...
function renderCompanyPanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="tab-bar">
      ...
      <button class="tab" onclick="switchTab(this,'tab-admins');renderAdminsTab(document.getElementById('tab-admins'))">Administradores</button>
...
function renderTrainingCard(t) {
  ...
  const mc=t.modules?.length||0,totalMin=(t.modules||[]).reduce((a,m)=>a+(parseInt(m.duration)||0),0),mins=Math.round(totalMin/60*10)/10;
  return '<div class="training-card" id="tc-'+t.id+'">'
    ...
    +'<div class="training-meta-row"><span style="font-size:.78rem;color:var(--tx3)">'+mc+' módulo'+(mc!==1?'s':'')+' · '+mins+'h</span></div></div>'
    ...
    +(t.modules||[]).map((m,i)=>'<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--sur2);border:1px solid var(--bor);border-radius:var(--r8);margin-bottom:6px">'
      ...
      +'<span style="font-family:var(--fm);font-size:.75rem;color:var(--tx3);white-space:nowrap">'+(m.duration ? Math.round(m.duration/60*10)/10+'h' : '—')+'</span>'
...
function addModuleRow(data) {
  ...
  row.innerHTML='<div class="drag-handle"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="7" r="1" fill="currentColor"/><circle cx="15" cy="7" r="1" fill="currentColor"/><circle cx="9" cy="12" r="1" fill="currentColor"/><circle cx="15" cy="12" r="1" fill="currentColor"/><circle cx="9" cy="17" r="1" fill="currentColor"/><circle cx="15" cy="17" r="1" fill="currentColor"/></svg></div>'
    ...
    +'<input class="mod-dur-input" type="number" min="0.5" max="24" step="0.5" placeholder="⏱ h" title="Carga horária em horas (ex: 1.5 = 1h30)" value="'+(data?.duration ? Math.round(data.duration/60*10)/10 : '')+'">'
...
async function renderAdminsTab(container) {
  if (!_fb) _fb = await import('./firebase.js');
  let emails = await _fb.getAdminEmails();
  if (!emails || !emails.length) emails = [...ADMIN_EMAILS];

  function renderList() {
    container.innerHTML = `
      <div style="max-width:580px">
        <p style="font-size:.85rem;color:var(--tx2);line-height:1.6;margin-bottom:14px">
          E-mails cadastrados aqui têm acesso ao painel admin e à auditoria.
          <strong style="color:var(--am)">Nunca remova seu próprio e-mail.</strong>
        </p>
        <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
          ${emails.map(email => `
            <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--sur2);border:1px solid var(--bor);border-radius:var(--r8)">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--tx3)" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              <span style="flex:1;font-family:var(--fm);font-size:.85rem;color:var(--tx)">${email}</span>
              <button onclick="removeAdmin('${email}')" class="btn btn-danger btn-xs">✕</button>
            </div>`).join('')}
        </div>
        <div style="display:flex;gap:8px">
          <input id="newAdminEmail" class="finput" type="email" placeholder="novo@empresa.com.br" style="flex:1"
  ...
  window.addAdmin = async () => {
    ...
    emails.push(email);
    ADMIN_EMAILS = [...emails];
    try { await _fb.saveAdminEmails(emails); toast('Admin adicionado!', 'ok'); } catch(e) { toast('Erro ao salvar', 'err'); return; }
    input.value = '';
    renderList();
  };

  window.removeAdmin = async (email) => {
    if (emails.length <= 1) { toast('Deve haver ao menos 1 admin', 'err'); return; }
    emails = emails.filter(e => e !== email);
    ADMIN_EMAILS = [...emails];
    try { await _fb.saveAdminEmails(emails); toast('Admin removido', 'ok'); } catch(e) { toast('Erro ao salvar', 'err'); return; }
    renderList();
  };
