/* ============================================================
   JCA Treinamentos — Módulo de Treinamento (Timer + Apresentação)
   ============================================================ */
...
const TrainingSession = (() => {
  ...
  function _loadProgress(trainingId, userId) {
    try {
      const key = `jca-treinamentos_progress_${userId}_${trainingId}`;
  ...
  function init(training, userId, resumeFromSaved = true) {
    ...
    _state.sessionKey = `jca-treinamentos_progress_${userId}_${training.id}`;
  ...
  return {
    ...
    getTimerState,
    formatSeconds,
    _forceComplete: (index) => {
      const mod = _state.training?.modules[index];
      if (mod) _state.completedModules.add(mod.id);
    },
    getState: () => ({ ..._state }),
  };
