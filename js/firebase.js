/* ============================================================
   JCA Treinamentos — Firebase Configuration
...
async function saveSession(sessionData) {
  const id = sessionData.userId + '_' + Date.now();
...
async function getSessions(filters = {}) {
  try {
    let q = collection(db, 'sessoes');
    const constraints = [];
    if (filters.companyId)    constraints.push(where('companyId',   '==', filters.companyId));
    if (filters.userId)       constraints.push(where('userId',      '==', filters.userId));
    if (filters.trainingId)   constraints.push(where('trainingId',  '==', filters.trainingId));
    if (constraints.length)   q = query(q, ...constraints);
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  } catch(e) { console.error('getSessions:', e); return []; }
}
